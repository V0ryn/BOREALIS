...
@onready var slide_mesh = $SlideHitbox/MeshInstance3D
...
func _unhandled_input(event):
	...
	if event.is_action_pressed("slide"):
		is_sliding = true
		print("is_sliding is now true")
		hitbox.disabled = true
		slide_hitbox.disabled = false
		$Hitbox/ShapeCast3D.collide_with_bodies == true
		print("Detecting collisions when uncrouched")
		
		slide_mesh.visible = true
		print("Slide mesh is visible")
		
		animator.play("StandToSlide")
		
		print("hitboxes changed successfully")
	
	elif event.is_action_released("slide"): #&& $Hitbox/ShapeCast3D.collide_with_bodies == false:
			is_sliding = false
			print("is_sliding is now false")
			hitbox.disabled = false
			slide_hitbox.disabled = true
			print("Hitboxes are now normal")
			
			slide_mesh.visible = false
			print("Slide mesh is invisible")
			
			animator.play("SlideToStand")
